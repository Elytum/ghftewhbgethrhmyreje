<?php
	...
	try {
		$conn = new PDO("mysql:host=$DB_DSN;port=$port;", $DB_USER, $DB_PASSWORD, array( PDO::ATTR_PERSISTENT => true));
		echo 'CREATE DATABASE';
		$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$create_database = $conn->prepare('CREATE DATABASE '.$dbname.';');
		$create_database->execute();

		$conn = new PDO("mysql:host=$DB_DSN;port=$port;dbname=$dbname", $DB_USER, $DB_PASSWORD, array( PDO::ATTR_PERSISTENT => true));
		$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$commands = [
